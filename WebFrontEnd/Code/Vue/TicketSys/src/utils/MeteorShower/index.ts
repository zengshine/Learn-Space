...
export class MeteorShower {
    ...
    animate() {
        ...
        //vm.terctx.clearRect(0, 0, vm.cvs.width, vm.cvs.height);
        vm.ctx.fillStyle = "#ffffff";
        vm.ctx.strokeStyle = "#ffffff";
        while (len--) {
            vm.animeObj[len].update(vm.cvs, vm.ctx);
        }
        this.requestId = requestAnimationFrame(vm.animate.bind(vm))
    }
    //绘制背景
